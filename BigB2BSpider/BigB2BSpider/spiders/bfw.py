...
class BaiFangWangSpider(CrawlSpider):
    name = "bfw"
    allowed_domains = ['byf.com','www.byf.com']
    start_urls = ['http://www.byf.com/b2b/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='Z_MenuList']//ul//li//a")), follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='list']//li//div[@class='proinfo fl']//a")), follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='page_news']//a[contains(text(),'下一页»')]")), follow=True),
    ...
    def parse_items(self, response):
        ...
        item["company_Name"] = response.xpath("//div[@class='l-main']//h1/text()").extract_first()
        ...
        item["telephone"] = response.xpath("//li[contains(text(),'公司电话：')]/text()").extract_first()
        item["phone"] = response.xpath("//li[contains(text(),'手机号码：')]/text()").extract_first()
