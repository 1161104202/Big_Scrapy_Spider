...
from BigB2BSpider.items import WeiLongShangWuWangspiderItem
from scrapy.cmdline import execute



class WeiLongShangWuWangSpider(CrawlSpider):
    name = 'wlsww'
    allowed_domains = ['cn.vlongbiz.com']
    start_urls = ['http://cn.vlongbiz.com/company/']
    cw = CleanWords()
    ...
    def parse_company_list(self, response):
        ...
        for tr in tr_list:
            item = WeiLongShangWuWangspiderItem()
            ...
            if city_infos:
                ...
                item["city_name"] = ''
            if company_href:
                if "http:" in company_href:
                # print(company_href)
                    contact_href = company_href + "contact/"
                else:
                    contact_href = "http://cn.vlongbiz.com" + company_href + "contact/"
                yield scrapy.Request(
                    url=contact_href,
        ...
        if next_page_url:
            if "http:" in next_page_url:
                ...
                next_page_url = "http://cn.vlongbiz.com" + next_page_url
