...
from BigB2BSpider.items import NongZiWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class NongZiWangSpider(CrawlSpider):
    name = "ampcn"
    allowed_domains = ['www.ampcn.com','ampcn.com']
    start_urls = ['http://www.ampcn.com/company/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r"search.asp\?daleiid=\d+",restrict_xpaths=("//tr//a")),follow=True),

        Rule(LinkExtractor(
            allow=r"\/show\/index.asp\?id=\d+", restrict_xpaths=("//font[@class='da']//a")),callback="parse_items",follow=True),
    ...
    def parse_items(self, response):
        ...
        item["company_Name"] = response.xpath("//td[contains(text(),'公司名称：')]/text()").extract_first()
        item["company_address"] = response.xpath("//td[contains(text(),'联系地址：')]/text()").extract_first()
        item["linkman"] = response.xpath("//td[contains(text(),'联 系 人：')]/text()").extract_first()
        item["telephone"] = response.xpath("//td[contains(text(),'联系电话：')]/text()").extract_first()
        item["phone"] = response.xpath("//td[contains(text(),'联系电话：')]/text()").extract_first()
        item["contact_Fax"] = response.xpath("//td[contains(text(),'联系电话：')]/text()").extract_first()
