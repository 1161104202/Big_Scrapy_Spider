...
from BigB2BSpider.items import CeKongWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class CeKongWangSpider(CrawlSpider):
    name = "ck365"
    allowed_domains = ['www.ck365.cn','ck365.cn']
    start_urls = ['http://www.ck365.cn/company/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='citysul']//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=(
                "//div[@id='m_left']//div[@class='list']//ul//li//a")), callback='parse_items',follow=True),
    ...
    def parse_items(self, response):
        ...
        item["company_Name"] = response.xpath("//ul[@class='com_contact']//b/text()").extract_first()
        item["company_address"] = response.xpath("//span[contains(text(),'址：')]/../text()").extract_first()
        item["linkman"] = response.xpath("//span[contains(text(),'联 系 人：')]/../text()").extract_first()
        item["telephone"] = response.xpath("//span[contains(text(),'公司电话：')]/../text()").extract_first()
        ...
        item["E_Mail"] = response.xpath("//span[contains(text(),'电子邮件：')]/../text()").extract_first()
        item["Source"] = response.url
        item["kind"] = response.xpath("//span[contains(text(),'主营行业：')]/../@title").get()
