...
from BigB2BSpider.items import DongShengDianCiWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class DongShengDianCiWangSpider(CrawlSpider):
    name = "battery"
    allowed_domains = ['battery.com.cn','corp.battery.com.cn']
    start_urls = ['http://corp.battery.com.cn/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=(
                "//div[@id='content']//div[@class='mainpanel l']//div[contains(@class,'corp')]//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=(
                "//div[@class='con']//div[@class='list']//ul//a")),follow=True),
    ...
    def parse_items(self, response):
        ...
        if item["company_Name"] and item["company_Name"] != '':
            ...
            try:
                item["company_Name"] = response.xpath("//div[@class='head']//h1/text()").get()
            except:
                return
