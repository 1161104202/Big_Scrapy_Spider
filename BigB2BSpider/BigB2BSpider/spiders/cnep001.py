...
class HuangBaoWangSpider(CrawlSpider):
    name = "cnep001"
    allowed_domains = ['www.cnep001.com','cnep001.com']
    start_urls = ['http://www.cnep001.com/company/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='dqdh']//li//a")), follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='pics']//div[@class='pictxt3 cle']//div[@class='btns'][2]/a")), callback='parse_items', follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='inc-page-jump']//a")), follow=True),
    ...
    def parse_items(self, response):
        ...
        item["company_Name"] = response.xpath("//li[contains(text(),'公司名称：')]/following-sibling::li/span/text()").extract_first()
        item["company_address"] = response.xpath("//li[contains(text(),'联系地址：')]/following-sibling::li/text()").extract_first()
        item["linkman"] = response.xpath("//li[contains(text(),'联 系 人：')]/following-sibling::li/text()").extract_first()
        item["telephone"] = response.xpath("//li[contains(text(),'联系电话：')]/following-sibling::li/text()").extract_first()
        item["phone"] = response.xpath("//li[contains(text(),'手　　机：')]/following-sibling::li/text()").extract_first()
        item["contact_Fax"] = response.xpath("//li[contains(text(),'联系传真：')]/following-sibling::li/text()").extract_first()
        item["contact_QQ"] = "".join(re.findall(pattern,response.text)) if re.findall(pattern,response.text) else ''
        item["E_Mail"] = response.xpath("//li[contains(text(),'电子邮件：')]/following-sibling::li/a/text()").extract_first()
        ...
        pattern_c = re.compile(r'>(.*?)</a> 版权所有　技术支持：<',re.S)
        pattern_k = re.compile(r'<meta name="keywords" content="(.*?)" />',re.S)
