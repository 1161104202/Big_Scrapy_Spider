...
from BigB2BSpider.items import FaShangJispiderItem
from scrapy.cmdline import execute



class FaShangJiSpider(CrawlSpider):
    name = 'fasj'
    allowed_domains = ['fashangji.com','www.fashangji.com']
    start_urls = ['https://www.fashangji.com/company/']
    cw = CleanWords()
    ...
    def parse(self, response):
        a_list = response.xpath("//div[@class='m m3']//div[contains(@class,'list')]//a")[0:593]
        for a in a_list:
            # print(a)
    ...
    def parse_company_list(self, response):
        div_list = response.xpath("//div[@class='m']//div[@class='company2']")
        for div in div_list:
            item = FaShangJispiderItem()
            item["company_Name"] = div.xpath(".//div[@class='company2-c']//a/text()").extract_first()
            company_href = div.xpath(".//div[@class='company2-c']//a/@href").extract_first()
            item["kind"] = div.xpath(".//div[@class='company2-c']//p[contains(text(),'经营范围：')]/text()").extract_first()
            item["company_address"] = div.xpath(".//div[@class='company2-c']//p[contains(text(),'公司地址：')]/text()").extract_first()
        ...
        if next_page_url:
            next_page_url = "https://www.fashangji.com" + next_page_url
            yield scrapy.Request(
                url=next_page_url,
                callback=self.parse_company_list
            )
