...
from BigB2BSpider.items import QiYe39spiderItem
...
class QiYe39Spider(CrawlSpider):
    ...
    def parse_company_list(self, response):
        ...
        for li in li_list:
            ...
            item["company_Name"] = li.xpath(".//a[@class='proName02']/text()").extract_first()
            company_href = li.xpath(".//a[@class='proName02']/@href").extract_first()
            if company_href:
                # print(company_href)
        ...
        next_page_url = response.xpath("//div[@class='listPage']//a[contains(text(),'下一页')]/@href").extract_first()
        if next_page_url:
    ...
    def parse_company_contact(self, response):
        ...
        item["kind"] = ""
        item["company_address"] = "".join(response.xpath("//th[contains(text(),'企业地址')]/following-sibling::td/text()").extract())
        item["linkman"] = "".join(response.xpath("//span[contains(text(),'联系人：')]/..//text()").extract())
        item["telephone"] = "".join(response.xpath("//span[contains(text(),'电话：')]/..//text()").extract())
        item["phone"] = "".join(response.xpath("//span[contains(text(),'手机：')]/..//text()").extract())
        item["E_Mail"] = "".join(response.xpath("//th[contains(text(),'电子邮件')]/following-sibling::td/text()").extract())
        item["contact_Fax"] = "".join(response.xpath("//th[contains(text(),'传真号码')]/following-sibling::td/text()").extract())
        item["contact_QQ"] = ""
        item["province"] = ''
        item["city_name"] = ''
