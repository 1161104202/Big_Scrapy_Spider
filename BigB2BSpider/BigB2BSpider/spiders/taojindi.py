...
from BigB2BSpider.items import TaoJinDiWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class TaoJinDiWangSpider(CrawlSpider):
    name = "taojindi"
    allowed_domains = ['hy.taojindi.com']
    start_urls = ['http://hy.taojindi.com/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='info-list info-list2']//ul//li//a")), follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='company-info']//li[@class='item']//h3//a")), callback='parse_items',follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//span[@class='pa_a']//a[contains(text(),'>')]")), follow=True),
    )

    def parse_items(self, response):
        item = TaoJinDiWangItem()
        pattern = re.compile(r'>联系人：(.*?)<',re.S)
        pattern1 = re.compile(r'>移动电话：(.*?)<',re.S)
        pattern2 = re.compile(r'>地址：(.*?)<',re.S)
        item["company_Name"] = response.xpath("//div[@class='wrap']//div[@class='hd']/h2/text()").get()
        ...
        item["linkman"] = "".join(re.findall(pattern,response.text)) if re.findall(pattern,response.text) else ''
