...
from BigB2BSpider.items import QinCaiHuangYeWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class QinCaiHuangYeWangSpider(CrawlSpider):
    name = "qincai"
    allowed_domains = ['www.qincai.com']
    start_urls = ['http://www.qincai.com/company']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='list-industry']//li//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='main_left']//li//strong//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='matpages']//a[contains(text(),'下一页')]")), follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=(
                "//div[@class='fr show nav']//a[contains(text(),'联系我们')]")), callback='parse_items', follow=True),

    )

    def parse_items(self, response):
        item = QinCaiHuangYeWangItem()
        pattern = re.compile(r'<meta name="keywords" content=".*?,(.*?)"/>',re.S)
        item["company_Name"] = response.xpath("//div[@class='companyname']/text()").extract_first()
        item["company_address"] = response.xpath("//span[contains(text(),'公司地址：')]/../text()").extract_first()
        item["linkman"] = response.xpath("//span[contains(text(),'联系人：')]/../text()").extract_first()
        item["telephone"] = response.xpath("//span[contains(text(),'电话号码：')]/../text()").extract_first()
        item["phone"] = response.xpath("///span[contains(text(),'手机号码：')]/../text()").extract_first()
        ...
        item["E_Mail"] = response.xpath("//span[contains(text(),'Email地址：')]/following-sibling::strong/a/text()").extract_first()
        item["Source"] = response.url
        item["kind"] = ",".join(response.xpath("//div[contains(text(),'主营产品：')]//text()").getall())
