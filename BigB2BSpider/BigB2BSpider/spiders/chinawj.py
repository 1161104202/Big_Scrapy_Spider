...
from BigB2BSpider.items import WuJinShangJiWangItem
...
class WuJinShangJiWangSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='proTypeList']//li//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=(
                "//div[@class='leftbox']//div[@class='pr0']//div[@class='pr2']//li[1]//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//p[@id='page']//a[contains(text(),'下一页')]")), follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=(
                "//div[@class='menu']//a[contains(text(),'联系方式')]")), callback='parse_items', follow=True),
    ...
    def parse_items(self, response):
        ...
        item["company_Name"] = response.xpath("//div[@class='head']//h1/text()").extract_first()
        ...
        item["telephone"] = "".join(re.findall(pattern3, response.text)[0]) if re.findall(pattern3, response.text) else ''
        item["phone"] = "".join(re.findall(pattern4, response.text)[0]) if re.findall(pattern4, response.text) else ''
        item["contact_Fax"] = "".join(re.findall(pattern5, response.text)[0]) if re.findall(pattern5,response.text) else ''
        item["contact_QQ"] = response.xpath("//img[@alt='跟我QQ洽谈']/../@href").get()
        item["E_Mail"] = "".join(re.findall(pattern6, response.text)) if re.findall(pattern6, response.text) else ''
        item["Source"] = response.url
        item["kind"] = "|".join(response.xpath("//div[@class='head']//h4//a//text()").extract())
