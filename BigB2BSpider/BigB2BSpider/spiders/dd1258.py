...
from BigB2BSpider.items import DingDingYiErWuBaWangItem
# from scrapy_redis.spiders import RedisCrawlSpider
from scrapy.cmdline import execute



class DingDingYiErWuBaWangSpider(CrawlSpider):
    name = "dd1258"
    allowed_domains = ['www.dd1258.com']
    start_urls = ['http://www.dd1258.com/company/']
    cw = CleanWords()
    ...
    rules = (
        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='city']//dl//dd//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=(
                "//div[@class='product_content']//ul//div[@class='company2']//div[@class='company2-c']//a")),follow=True),

        Rule(LinkExtractor(
            allow=r".*", restrict_xpaths=("//div[@class='pages']//a[contains(text(),'下一页»')]")), follow=True),

        Rule(LinkExtractor(
            allow=r".*",restrict_xpaths=("//div[@class='wrap']//span//a[contains(text(),'联系方式')]")), callback='parse_items', follow=True),

    )

    def parse_items(self, response):
        item = DingDingYiErWuBaWangItem()
        pattern = re.compile(r'<meta name="keywords" content=".*?,(.*?)"/>',re.S)
        item["company_Name"] = response.xpath("//dt[contains(text(),'公司名称：')]/following-sibling::dd/text()").extract_first()
        item["company_address"] = response.xpath("//dt[contains(text(),'公司地址：')]/following-sibling::dd/a/text()").extract_first()
        item["linkman"] = response.xpath("//dt[contains(text(),'联 系 人：')]/following-sibling::dd/text()").extract_first()
        item["telephone"] = response.xpath("//dt[contains(text(),'公司电话：')]/following-sibling::dd/text()").extract_first()
        item["phone"] = response.xpath("//dt[contains(text(),'手机号码：')]/following-sibling::dd/text()").extract_first()
        item["contact_Fax"] = response.xpath("//dt[contains(text(),'公司传真：')]/following-sibling::dd/text()").extract_first()
        item["contact_QQ"] = response.xpath("//img[@title='点击QQ交谈/留言']/../@href").extract_first()
        item["E_Mail"] = response.xpath("//dt[contains(text(),'电子邮件：')]/following-sibling::dd/text()").extract_first()
